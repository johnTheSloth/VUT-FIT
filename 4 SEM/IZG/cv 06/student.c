...
/// Shininess faktor nastavte na 40.f.<br/> - nemam ????
...
"  float dF = clamp(dot(Normal,L),0.f,1.f);\n" // skalar soucitn a orez do 0,1

"  vec3 V = normalize(cameraPos - FragPos);\n" //viz vyse, jen jine params
"  vec3 R = reflect(-V,Normal);\n"

"  float sF = clamp(dot(R,L),0.f,1.f);\n" // viz vzorec, umocneni samostatne
"  sF = pow(sF, 40);\n"

"  vec3 phong = dF * dM * dL + sF * sM * sL;\n" //barva, musim az nakonec, kdy mam vsechny promenny
...
void phong_onInit(int32_t width,int32_t height){
  ...
  GLuint normal_attr = (GLuint) glGetAttribLocation(phong.program, "normal");
  glVertexAttribPointer(normal_attr, 3, GL_FLOAT, GL_FALSE, sizeof(BunnyVertex), (GLvoid const*) (sizeof(float)*3));
...
void phong_onDraw(){
  ...
  glUniformMatrix4fv(phong.projectionMatrixUniform, 1/*sizeof(bunnyIndices)*/, GL_FALSE, (float*)&projectionMatrix);
  glUniformMatrix4fv(phong.viewMatrixUniform, 1/*sizeof(bunnyIndices)*/, GL_FALSE, (float*)&viewMatrix);
  ...
  GLint lightPos = glGetUniformLocation(phong.program, "lightPos");
  GLint cameraPos = glGetUniformLocation(phong.program, "cameraPos");
  glUniform3f(lightPos, phong.lightPosition.data[0], phong.lightPosition.data[1], phong.lightPosition.data[2]);
  glUniform3f(cameraPos, cameraPosition.data[0], cameraPosition.data[1], cameraPosition.data[2]);
  glBindVertexArray(0);
}
