...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  (void)head;
  (void)gl_VertexID;
  size_t g = head->buffer;
  g += (head->offset + gl_VertexID * head->stride);
  return (uint8_t*)g;
}
...
void gpu_runVertexPuller(
  ...
  {
	  if (puller->heads == NULL || (&puller->heads[i])->enabled == 0) {
			output->attributes[i] = NULL;
	  }else if ((&puller->heads[i])->enabled) {
			output->attributes[i] = (const void*)gpu_computeVertexAttributeDataPointer(&puller->heads[i], gpu_computeGLVertexID(puller->indices, vertexShaderInvocation));
	  }
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  ///  - GPUVertexShaderInput()
	...
	GPUVertexShaderInput vsi;
	for (size_t i = 0; i < nofPrimitiveVertices; i++)
	{
		gpu_runVertexPuller(&vpo, puller, baseVertexShaderInvocation + i);
		vsi.attributes = &vpo;
		vsi.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + i);
		primitive->nofUsedVertices = nofPrimitiveVertices;
		vertexShader(primitive->vertices+i, &vsi, gpu);
	}
}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  coords->data[0] = ((vertices[1].data[1] - vertices[2].data[1])*(pixelCenter->data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0])*(pixelCenter->data[1] - vertices[2].data[1])) / det;
  coords->data[1] = ((vertices[2].data[1] - vertices[0].data[1])*(pixelCenter->data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0])*(pixelCenter->data[1] - vertices[2].data[1])) / det;
