...
void	bezierDeCasteljau(int quality, const S_Vector *points, S_Vector *line_points)
{
  ...
  double n = point2d_vecSize(points) - 1.0;
  double t = 0;
  ...
  S_Vector *localVec = point2d_vecCreateEmpty();

  if (n < 0) return;

  for (int q = 0; q <= quality; ++q, t+=t_step)
  {
	  //zkopiruj vstupni vektor do lokalniho vektoru bodu
	  localVec = vecDeepCopy(points);
	  for (int j = 0; j < n; ++j)
	  {
		  for (int i = 0; i < n-j; ++i)
		  {
			  mullPoint2d((1 - t), point2d_vecGetPtr(localVec, i), &temp1);
			  mullPoint2d(t, point2d_vecGetPtr(localVec, i + 1), &temp2);

			  addPoint2d(&temp1, &temp2, &result);
			  point2d_vecSet(localVec, i, result);
		  }
	  }
	  point2d_vecPushBack(line_points, result);
	  point2d_vecRelease(&localVec);
  }
