...
**/
	...
	tBTNodePtr parent = NULL, child = *RootPtr;
	bool isFound = false;

	while(child != NULL && !isFound) {
		parent = child;							//postupne "posouvani" se ve stromu
		if(parent->Cont > Content) {			//rozhodnuti na kterou stranu ve stromu se "posunu"
			child = parent->LPtr;
		}else if (parent->Cont < Content) {
			child = parent->RPtr;
		}else {									//content se shoduje, prvek byl nalezen
			isFound = true;
		}
	}

	if(!isFound) {								//prvek nebyl nalezen
		tBTNodePtr temp = malloc(sizeof(struct tBTNode));
		if(temp != NULL) {						//vytvori se novy s pozadovanymi hodnotami
			temp->LPtr = NULL;
			temp->RPtr = NULL;
			temp->Cont = Content;

			if(parent == NULL) {				//pokud byl RootPtr NULL, uzel se vlozi uplne na zacatek
				*RootPtr = temp;
			}else if(parent->Cont > Content) {	//pripojeni na rodice zleva
				parent->LPtr = temp;
			}else {								//pripojeni zprava
				parent->RPtr = temp;
			}
		}
	}
...
**/
	...
	tStackP stack;						//vytvorim a inicializuji zasobnik
	SInitP(&stack);
	tBTNodePtr temp = RootPtr;

	Leftmost_Preorder(temp, &stack);	//pruchod nejlevejsimi uzly, naplneni zasobniku

	while(!SEmptyP(&stack)) {			//kdyz neni zasobnik prazdny
		temp = STopPopP(&stack);				//nacteni ze zasobniku
		Leftmost_Preorder(temp->RPtr, &stack);	//pruchod nejlevejsimi uzly
	}
}
...
**/
	...
	tStackP stack;					//vytvorim a inicializuju zasobnik
	SInitP(&stack);
	tBTNodePtr temp = RootPtr;

	Leftmost_Inorder(temp, &stack);	//pruchod nejlevejsimi uzly, naplneni zasobniku

	while(!SEmptyP(&stack)) {		//kdyz neni zasobnik prazdny
		temp = STopPopP(&stack);	//nacteni ze zasobniku
		Leftmost_Inorder(temp->RPtr, &stack);	//pruchod nejlevejsimi uzly
		BTWorkOut(temp);			//print
	}
}
...
**/
	...
	while(ptr != NULL) {
		SPushP(StackP, ptr);		//vlozeni uzlu do zasobniku
		SPushB(StackB, true);		//vlozeni bool do zasobniku
		ptr = ptr->LPtr;			//posun v cyklu
	}
}
...
**/
	...
	tStackP stackP;			//vytvorim a inicializuju zasobnik pro uzly
	SInitP(&stackP);

	tStackB stackB;			//vytvorim a inicializuji zasobnik pro bool hodnoty
	SInitB(&stackB);

	tBTNodePtr temp = RootPtr;

	Leftmost_Postorder(temp, &stackP, &stackB);	//pruchod nejlevejsimi uzly, naplneni obou zasobniku

	while(!SEmptyP(&stackP)) {					//dokud zasobnik s uzly neni prazdny
		temp = STopPopP(&stackP);				//ziskani hodnoty vrcholu zasobniku
		SPushP(&stackP, temp);					//navrat zpatky na vrchol

		if(STopPopB(&stackB)) {					//pokud uz jsme uzel 1 prohlidli
			SPushB(&stackB, false);				//hodnota na zasobniku se nastavi na false
			Leftmost_Postorder(temp->RPtr, &stackP, &stackB);	//pruchod nejlevejsimi uzly praveho podstromu
		}else {									//uzel uz se prohlizel vicekrat, protoze bool hodnota je false
			temp = STopPopP(&stackP);			//popnuti uzlu ze zasobniku
			BTWorkOut(temp);
		}
	}
}
...
**/
	...
	tBTNodePtr temp;
	tStackP stack;				//vytvorim a inicializuji zasobnik pro uzly
	SInitP(&stack);

	if((*RootPtr) != NULL) {
		do{								//proved alespon 1x dokud neni zasobnik uzlu prazdny nebo se ukazatel na koren nerovna NULL
			if((*RootPtr) != NULL) {
				if((*RootPtr)->RPtr != NULL) {			//pokud pravy podstrom neni NULL, vloz ho na zasobnik
					SPushP(&stack, (*RootPtr)->RPtr);
				}

				temp = *RootPtr;						//prirazeni pointru na koren pro uvolneni
				*RootPtr = (*RootPtr)->LPtr;
				free(temp);
			}else {										//RootPtr je NULL ale strom jeste nebyl kompletne zrusen
				if (!SEmptyP(&stack)) {					//pokud neni zasobnik prazdny, prenastavi se ukazatel korene
