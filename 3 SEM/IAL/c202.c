...
*/
	...
	if(s == NULL) {
		...
		s->top = -1;							//z komentare nahore vim ze pokud je zasobnik prazdny, ma byt hodnota ->top -1
	}
...
*/
	...
	return (s->top == -1) ? 1 : 0; 				// Je prazdny? ano - return 1, ne - return 0
...
*/
	...
	return (s->top == STACK_SIZE-1) ? 1 : 0; 	// Je plny? ano - return 1, ne - return 0
...
*/
	...
	if(stackEmpty(s)) {
		...
		*c = s->arr[s->top];		//priradi do c znak, index vezme z parametru top
	}
...
*/
	...
	if(!stackEmpty(s)) {
		s->top--;				//snizeni indexu prvku vrcholu zasobniku
	}
...
*/
	...
	if(stackFull(s)) {
		...
		s->top++;				//zvyseni indexu prvku vrcholu zasobniku
		s->arr[s->top] = c;		//vlozeni znaku na vrchol s drive zvysenym indexem
	}
